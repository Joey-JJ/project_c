...
const AccountInfo = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (newEmail) {
      ...
      newProfile.charge_card != profile?.charge_card &&
      newProfile.charge_card != null
    ) {
      if (checkDoubleHyphen(newProfile.charge_card)) {
        if (
          testCardNumber(
            validChargeNumber,
            newProfile.charge_card.replace(/-/g, "").toUpperCase()
          )
        ) {
      ...
      newProfile.license_number != profile?.license_number &&
      newProfile.license_number != null
    ) {
      if (checkDoubleHyphen(newProfile.license_number)) {
        if (
          testLicenseNumber(
            validLicenseNumbers,
            newProfile.license_number.toUpperCase()
          )
        ) {
      ...
      newProfile.full_name != profile?.full_name &&
      ...
      <div className="container mx-auto min-h-screen flex flex-col mt-8 mb-8 items-center">
        ...
        <div className="stats shadow stats-vertical">
          <div className="stat-value text-sm text-center">
            Please edit one detail at a time
          </div>
          <div className="stat">
          ...
          <div className="stat">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="stat-actions">
                <button type="submit" className="btn-primary btn-sm btn-wide">
                  Edit
                </button>
              </div>
            </form>
